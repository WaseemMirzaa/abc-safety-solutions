...
const featureItems = [
  {
    icon: BookOpen,
    t: 'Structured modules',
    d: 'Slide + audio flows mirror the rigor of your Houston classroom programs.',
  },
  {
    icon: Shield,
    t: 'Knowledge checks',
    d: 'Short assessments gate completion so credentials mean something on site.',
  },
  {
    icon: Award,
    t: 'Instant credentials',
    d: 'PDF certificates (via API later) + dashboard storage for audits and renewals.',
  },
] as const
        ...
        <Container className="relative py-12 sm:py-16 lg:py-20">
          <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-12 lg:items-center xl:gap-14">
            ...
            >
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-sky-300/70 bg-gradient-to-r from-sky-50 to-sky-100/80 px-3.5 py-1.5 text-xs font-semibold text-sky-900 shadow-sm">
                ...
                Self-paced · Tested · Certificate-ready
              </div>
              <h1 className="mt-5 max-w-2xl font-display text-[1.75rem] font-bold leading-[1.12] tracking-tight sm:text-4xl sm:leading-[1.1] lg:text-[2.35rem] xl:text-[2.6rem]">
                ...
                <span className="text-brand-900">for safer, compliant worksites.</span>
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-relaxed text-slate-600 sm:text-base">
                Our goal is to get your team back to working safely, quickly, and in compliance with the latest regulations. This portal adds
                self-paced online modules—slides, knowledge checks, and certificates—to the same disciplines we teach in the classroom and on
                site in Houston and nationwide.
              </p>
              <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center">
                <Link to="/courses">
                  <Button className="w-full min-w-[12rem] px-8 sm:w-auto">Explore catalog</Button>
                </Link>
                <Link to="/register">
            ...
            >
              ...
              <div className="relative aspect-[4/5] max-h-[min(72vh,440px)] w-full overflow-hidden rounded-3xl border-2 border-sky-200/90 bg-white shadow-[0_20px_50px_-20px_rgba(14,165,233,0.35),0_24px_60px_-28px_rgba(15,23,42,0.12)] ring-2 ring-sky-300/30 sm:aspect-[3/4] sm:max-h-[min(68vh,480px)] lg:max-h-[min(75vh,520px)] lg:aspect-[4/5]">
                <img
                  src={homeHeroImage}
                  alt="Occupational Safety & Health Training — ABC Safety Solutions"
                ...
                <div className="absolute inset-x-0 bottom-0 p-5 sm:p-6">
                  <p className="font-display text-xs font-semibold uppercase tracking-[0.2em] text-sky-200/95">Occupational Health & Safety</p>
                  <p className="mt-2 font-display text-lg font-semibold leading-snug text-white sm:text-xl">
                    Classroom, simulator, and onsite programs—plus online completion tracking.
                  </p>
                </div>
              </div>
              <div className="absolute -bottom-2 left-3 right-auto z-10 sm:-bottom-3 sm:left-5">
                <div className="flex items-center gap-2 rounded-2xl border border-sky-200/90 bg-white/95 px-3 py-2 shadow-md shadow-sky-900/10 backdrop-blur-md sm:gap-3 sm:px-4 sm:py-2.5">
                  ...
                  <div className="min-w-0 text-left">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-sky-700">Completion</p>
                    <p className="truncate text-sm font-semibold text-brand-900 sm:text-base">Certs & progress tracked</p>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <div className="mt-10 max-w-4xl rounded-2xl border border-sky-200/80 bg-gradient-to-r from-sky-600/[0.08] via-white/90 to-sky-500/[0.06] px-4 py-4 shadow-sm shadow-sky-900/5 sm:px-8 sm:py-5 lg:mt-12">
            <dl className="grid grid-cols-3 gap-4 divide-x divide-sky-200/60 text-center sm:gap-6">
              {[
                { k: 'Courses', v: isPending ? '…' : `${courses.length}+`, d: 'Launch catalog' },
                { k: 'Format', v: 'Slides', d: '+ audio + test' },
                { k: 'Wallet', v: 'Certs', d: 'Web & app ready' },
              ].map((x) => (
                <div key={x.k} className="px-2 first:pl-0 last:pr-0">
      ...
      <section className="relative border-y border-sky-100/80 bg-gradient-to-b from-sky-50/40 to-white py-16 sm:py-20">
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            <p className="font-display text-xs font-semibold uppercase tracking-[0.25em] text-sky-600">Programs</p>
            <h2 className="mt-3 font-display text-2xl font-bold leading-tight text-brand-900 sm:text-3xl md:text-[2rem]">
              Your one stop shop training provider for all your training and compliance needs.
            </h2>
            <p className="mt-4 text-sm leading-relaxed text-slate-600 sm:text-base">
              Over 50 health, safety, and specialty skillset programs—construction, manufacturing, oil and gas, and more. Images below match
              the categories on{' '}
              <a
                ...
                rel="noreferrer"
              >
                abcsafetysolutions.com/training-certification
              </a>
              .
            </p>
          </div>
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {trainingProgramTiles.map((prog) => (
              <Link
                key={prog.title}
                to="/courses"
              ...
              >
                ...
                <div className="p-4">
                  <p className="font-display text-sm font-semibold leading-snug text-brand-900">{prog.title}</p>
                  {'note' in prog && prog.note ? (
                    <p className="mt-2 text-[11px] leading-relaxed text-slate-500">{prog.note}</p>
                  ) : null}
                </div>
              </Link>
            ))}
      ...
      <section className="relative border-y border-slate-200/80 bg-white py-16 sm:py-20">
        <Container>
          <div className="mx-auto max-w-2xl text-center">
            <p className="font-display text-xs font-semibold uppercase tracking-[0.25em] text-sky-600">Why teams choose it</p>
            <h2 className="mt-3 font-display text-2xl font-bold text-brand-900 sm:text-3xl">Clarity, proof, and zero friction</h2>
          </div>
          <div className="mt-14 grid gap-6 lg:grid-cols-3">
            {featureItems.map((x, i) => (
              ...
              >
                ...
                <h3 className="mt-6 font-display text-lg font-semibold text-brand-900">{x.t}</h3>
                <p className="mt-3 text-sm leading-relaxed text-slate-600">{x.d}</p>
              </motion.div>
            ))}
          </div>
        </Container>
        ...
        <Container>
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            <div className="max-w-xl">
              <p className="font-display text-xs font-semibold uppercase tracking-[0.25em] text-amber-700">Featured</p>
              <h2 className="mt-2 font-display text-3xl font-bold text-brand-900 sm:text-4xl">Popular online courses</h2>
              <p className="mt-4 text-slate-600">
                Same disciplines as onsite training—packaged for remote completion with a polished learner experience.
              </p>
            </div>
            <Link
              to="/courses"
