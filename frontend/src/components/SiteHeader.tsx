...
function linkClass({ isActive }: { isActive: boolean }) {
  return clsx(
    'rounded-full px-3.5 py-2 text-sm font-medium capitalize transition-colors duration-200',
    isActive
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/90 bg-white/90 backdrop-blur-xl backdrop-saturate-150">
      <Container className="flex min-h-[5.25rem] min-w-0 items-center justify-between gap-2 py-2 sm:min-h-[5.75rem] sm:gap-4 sm:py-2.5">
        <Link
          to="/"
          className="group flex min-w-0 max-w-[min(100%,14rem)] shrink items-center gap-2 py-1 pr-1 sm:max-w-none sm:gap-3 sm:pr-2"
        >
          <span className="flex h-[3.25rem] shrink-0 items-center sm:h-[3.75rem]">
            <img
        ...
        <div className="hidden items-center gap-2 md:flex">
          {user ? (
            <>
              ...
              <Button variant="secondary" className="!rounded-full !py-2 !text-xs !font-medium capitalize" onClick={() => logout()}>
            ...
            <>
              <Link to="/login">
                <Button variant="ghost" className="!rounded-full !px-4 capitalize">
                  {t('ui_site_header_sign_in')}
                </Button>
              </Link>
              <Link to="/register">
                <Button className="!rounded-full !px-5 !py-2 !text-sm capitalize">{t('SiteHeader_93_get_started_dc35ebe597')}</Button>
              </Link>
            </>
          )}
        </div>
          ...
          <Container className="flex flex-col gap-1 py-4">
            {navPublic.map((n) => (
              <NavLink
                key={n.to}
                to={n.to}
                className="rounded-xl px-4 py-3.5 capitalize text-slate-800"
            ...
            <NavLink
              to="/notifications"
              className="flex items-center gap-2 rounded-xl px-4 py-3.5 capitalize text-slate-800"
              onClick={() => setOpen(false)}
            >
              <div className="relative flex items-center justify-center">
                <Bell className="h-4 w-4" />
            ...
            {user
              ...
              : null}
            {user?.role === 'admin' ? (
              <NavLink to="/admin" className="rounded-xl px-4 py-3.5 capitalize text-slate-800" onClick={() => setOpen(false)}>
                {t('ui_site_header_admin')}
              </NavLink>
            ) : null}
            ...
            {user ? (
              <button
                type="button"
                className="rounded-xl px-4 py-3.5 text-left capitalize text-slate-600"
                onClick={() => {
                  logout()
                  setOpen(false)
                }}
              ...
              <>
                <Link to="/login" className="rounded-xl px-4 py-3.5 capitalize text-slate-800" onClick={() => setOpen(false)}>
                  {t('ui_site_header_sign_in')}
                </Link>
                <Link to="/register" className="rounded-xl px-4 py-3.5 capitalize font-semibold text-sky-800" onClick={() => setOpen(false)}>
