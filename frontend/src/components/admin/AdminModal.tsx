...
type Props = {
  ...
  onClose: () => void
  wide?: boolean
  /** Block backdrop / escape close while a file upload is in progress. */
  blockDismiss?: boolean
}

export function AdminModal({ title, children, onClose, wide, blockDismiss }: Props) {
  return (
    <div
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-3 sm:gap-4">
          ...
          <button
            type="button"
            className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-brand-900 disabled:cursor-not-allowed disabled:opacity-40"
